...
import { Terrain, dayDateToDate } from "./bootstrapHandler/bootstrapModel";
export default {
    /**
    ...
    async bootstrap({ strapi }) {
        ...
        const terrains = JSON.parse(readFileSync("./bootstrapData.json", {encoding:'utf-8'})) as Terrain[];
            ...
            if(!strapiTerrain){
                ...
                for (const userAuthorPair of terrain.users) {
                    let newUser = await strapi.plugins['users-permissions'].services.user.add({
                        ...
                        password: userAuthorPair.user.email, //will be hashed automatically
